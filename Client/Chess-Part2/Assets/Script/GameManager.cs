...
public class GameManager : MonoBehaviour {
	...
	public GameObject GetPiece(string str){
	...
	void AddPieces()
	{
		...
		for(int i = 0; i < _boardSize; i++)
		{
			temp = CreatePiece("Pawn", i, 1, 1); // Create all white pawn
			dictionary.Add("Pawn" + i.ToString() + "W", temp);
			temp = CreatePiece("Pawn", i, 6, -1); // Create all dark pawn
			dictionary.Add("Pawn" + i.ToString() + "B", temp);
		}
		
		// Create white pieces
		_linePosY = 0;
		_piecePlayer = 1;
		temp = CreatePiece("Rook"  , 0, _linePosY, _piecePlayer);
		dictionary.Add ("Rook0W", temp);
		temp = CreatePiece("Knight", 1, _linePosY, _piecePlayer);
		dictionary.Add ("Knight0W", temp);
		temp = CreatePiece("Bishop", 2, _linePosY, _piecePlayer);
		dictionary.Add ("Bishop0W", temp);
		temp = CreatePiece("Queen" , 3, _linePosY, _piecePlayer);
		dictionary.Add ("QueenW", temp);
		temp = CreatePiece("King"  , 4, _linePosY, _piecePlayer);
		dictionary.Add ("KingW", temp);
		temp = CreatePiece("Bishop", 5, _linePosY, _piecePlayer);
		dictionary.Add ("Bishop1W", temp);
		temp = CreatePiece("Knight", 6, _linePosY, _piecePlayer);
		dictionary.Add ("Knight1W", temp);
		temp = CreatePiece("Rook"  , 7, _linePosY, _piecePlayer);
		dictionary.Add ("Rook1W", temp);
		
		// Create Dark pieces
		_linePosY = 7;
		_piecePlayer = -1;
		temp = CreatePiece("Rook"  , 0, _linePosY, _piecePlayer);
		dictionary.Add ("Rook0B", temp);
		temp = CreatePiece("Knight", 1, _linePosY, _piecePlayer);
		dictionary.Add ("Knight0B", temp);
		temp = CreatePiece("Bishop", 2, _linePosY, _piecePlayer);
		dictionary.Add ("Bishop0B", temp);
		temp = CreatePiece("Queen" , 3, _linePosY, _piecePlayer);
		dictionary.Add ("QueenB", temp);
		temp = CreatePiece("King"  , 4, _linePosY, _piecePlayer);
		dictionary.Add ("KingB", temp);
		temp = CreatePiece("Bishop", 5, _linePosY, _piecePlayer);
		dictionary.Add ("Bishop1B", temp);
		temp = CreatePiece("Knight", 6, _linePosY, _piecePlayer);
		dictionary.Add ("Knight1B", temp);
		temp = CreatePiece("Rook"  , 7, _linePosY, _piecePlayer);
		dictionary.Add ("Rook1B", temp);
	}
	
	// Spawn a piece on the board
	GameObject CreatePiece(string _pieceName, int _posX, int _posY, int _playerTag)
	{
	...
	bool TestMovement(GameObject _SelectedPiece, Vector2 _coordToMove)
	{
		...
		GameObject game = FindAt (new Vector3 (_coordToMove.x, 0.0f, _coordToMove.y));
		if (game != null) {
			if (_SelectedPiece.CompareTag (game.tag)) {
				_collisionDetectBool = true;
				Debug.Log("Collide with own!");
			}
		}
